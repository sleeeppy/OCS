"""Create an OCS project from an existing see-through PSD, skipping the GPU pass.
...
Expects see-through's normal output layout next to the PSD:
...
def main() -> int:
    ...
    ap.add_argument("psd", type=Path, help="see-through PSD to import")
    ap.add_argument("--name", default=None, help="project name (defaults to the PSD stem)")
    args = ap.parse_args()

    psd = args.psd.resolve()
    if not psd.exists():
        print(f"error: {psd} not found", file=sys.stderr)
        return 1

    ensure_dirs()
    name = args.name or psd.stem
    ...
    shutil.copy2(psd, root / "seethrough" / psd.name)
    for extra in (Path(str(psd) + ".json"), psd.with_name(psd.stem + "_depth.psd")):
        if extra.exists():
            shutil.copy2(extra, root / "seethrough" / extra.name)
    src_dir = psd.with_suffix("")
    if src_dir.is_dir():
        shutil.copytree(src_dir, root / "seethrough" / src_dir.name, dirs_exist_ok=True)

    decomp = psd_io.read_decomposition(root / "seethrough" / psd.name)
    ...
    project = pipeline.Project(id=pid, root=root, state={
        ...
        "message": "imported",
        "settings": {},
        "input": {"filename": psd.name, "width": decomp.canvas[0],
                  "height": decomp.canvas[1], "had_alpha": None},
        "psd": str((root / "seethrough" / psd.name).relative_to(root)),
